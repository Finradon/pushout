...
use matchbox::{Rule, find_matches, apply_once, Morphism};
...
#[test]
fn test_add_edge_rule() {
    // Host graph: nodes P, Q (no edge P->Q)
    let mut host = Graph::<&str, &str>::new();
    let hp = host.add_node("P");
    let hq = host.add_node("Q");

    // Rule: add edge P->Q
    let mut lhs = Graph::<&str, &str>::new();
    let lp = lhs.add_node("P");
    let lq = lhs.add_node("Q");

    let mut interface = Graph::<&str, &str>::new();
    let ip = interface.add_node("P");
    let iq = interface.add_node("Q");

    let mut rhs = Graph::<&str, &str>::new();
    let rp = rhs.add_node("P");
    let rq = rhs.add_node("Q");
    rhs.add_edge(rp, rq, "pq");

    let mut l2k = Morphism::new();
    l2k.insert_node(lp, ip);
    l2k.insert_node(lq, iq);
    let mut k2r = Morphism::new();
    k2r.insert_node(ip, rp);
    k2r.insert_node(iq, rq);
    let rule = Rule::new(lhs, interface, rhs, l2k, k2r);

    let matches = find_matches(&rule, &host, true);
    assert_eq!(matches.len(), 1, "Should find one match for node pair");

    let result = apply_once(&rule, &host, &matches[0]).expect("apply_once should succeed");
    assert!(result.find_edge(hp, hq).is_some(), "Edge P->Q was added");
}

#[test]
fn test_gluing_failure() {
    // Host graph: A->B->C
    let mut host = Graph::<&str, &str>::new();
    ...
    // Rule: delete node B (no interface)
    let mut lhs = Graph::<&str, &str>::new();
    let _lb = lhs.add_node("B");
    let interface = Graph::<&str, &str>::new();
    let rhs = Graph::<&str, &str>::new();
    let l2k = Morphism::new();
    let k2r = Morphism::new();
    let rule = Rule::new(lhs, interface, rhs, l2k, k2r);

    let matches = find_matches(&rule, &host, true);
    assert_eq!(matches.len(), 1, "Should find one match for node B");

    let result = apply_once(&rule, &host, &matches[0]);
    assert!(result.is_none(), "apply_once should fail due to dangling edge");
}
